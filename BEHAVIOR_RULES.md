...
- 🚨 **ADDITIVE-ONLY DATABASE / Hook 18** (VM Rule 15) — DDL discipline + the VM pre-commit hook that enforces it. The Hub issues no DDL, so there is no DDL surface here to protect. ⚠️ **The rest of this bullet used to read "(it reads a read-only replica)" and stop there — that is no longer the whole truth, and the correction is the point of this entry.** As of **D2 (2026-07-31) the Hub has ONE write path to the live `trevor.db`**: the digest feed's per-card delete. It does **not** write the replica (0444; a write there is refused outright and would silently revert on the next ~20-min tailsync) — it reaches the VM live DB over the `ssh vm` pipe via `set_digest_delete.py`. **This box now has a `DELETE` surface, and Hook 18 does not run here** (WSL has no pre-commit guard chain, deliberately — see `CLAUDE.md`), so the discipline below is enforced by review, not by a hook.

  **The exception text, mirrored VERBATIM from the canonical VM `BEHAVIOR_RULES.md` Rule 15 (commit `1ed5f2a`, 2026-07-31). Do not paraphrase it here; if the VM text changes, re-copy it.**

  > **Exceptions (exhaustive — each requires explicit Ghost approval; none generalizes):**
  > 1. **`recon_archive` rolling FIFO prune** — `auto_trader/recon_archive.py` `_prune_db_rolling`/`_prune_files_rolling`, a cap-bounded prune of an archive index. Pre-existing standing exception.
  > 2. **`digest` table only** (Ghost-directed, 2026-07-31, D1). `digest` is an observability artifact created 2026-07-30, read by nothing on the trading path, holding no trading state. Bounded DELETEs by named `digest_date` are permitted; an unbounded `DELETE FROM digest` is not. **This never generalizes** — no other table gains a DELETE path from it, and any future DELETE elsewhere needs its own explicit Ghost approval.

  **Hub-side implementation of exception #2 (D2):** `set_digest_delete.py` → `DELETE /api/health/digests/<date>`. Bounded three ways — a strict `^\d{4}-\d{2}-\d{2}$` shape gate in the route *and* the helper, a pre-count that issues **no** DELETE at all on 0 or >1 matches, and a post-DELETE `rowcount != 1` rollback. `digest_date` is a bound SQL parameter, never interpolated. Writes an additive `change_log` audit row in the same transaction. **There is no code path here to an unbounded delete, and none may be added.**

  ⚠️ **Why this is `ssh`-based and not a gateway op, recorded so a future reader does not read it as an oversight:** the two-hop `:3939 → :3940` write-gateway is a **fixed op allowlist**, each op bound to a helper file that must exist **on the VM**; `digest.delete` returns `unknown op` (400, measured live at the D2 gate). Adding it is VM-side work. **The gateway is the cleaner long-term home** — it carries a flag gate and a mandatory idempotency key, neither of which the `ssh` path has. **If the Hub ever grows more write paths, move this one into the gateway rather than adding a second `ssh`-shaped writer.**
