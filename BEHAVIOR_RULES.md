...
### Rule — `NEXT_PUBLIC_LIVE_TERMINAL` gates the browser-side live-price overlay (RM-LIVE, default OFF)
The live-terminal feature is an **additive, read-only display overlay** that streams live marks straight from Hyperliquid's public `allMids` WebSocket **in the client browser** — no server, gateway, VM, or HL-REST call is involved (no VM load, no money path). It is gated by the build-time env flag **`NEXT_PUBLIC_LIVE_TERMINAL`** (a Next.js `NEXT_PUBLIC_*` var, read client-side), **default OFF** (unset or `0`). Foundation (Round 1): the dormant store `src/lib/hl-ws-store.ts` (subscriber-counted singleton WS client + `useLiveMark(ticker, enabled)` hook), the `<LiveValue>` primitive (B2), and this flag (B3). Consumers land in Round 2 and MUST call `useLiveMark` unconditionally with `enabled={liveTerminalFlag}` (React hook rules) so that **flag-OFF opens no socket and is byte-identical to the Hub today**. **Rollback = unset the flag or set it to `0`** and rebuild → the overlay vanishes, the underlying SWR/REST surfaces (`/api/prices`, etc.) are untouched. Ticker keying mirrors `/api/prices` exactly (case-insensitive match, caller-casing preserved, `kPEPE`-safe). Reconnect/backoff for the WS is a separate item (B7) — not part of the foundation.
