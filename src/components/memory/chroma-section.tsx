    ...
    <div className="space-y-4 p-4 md:space-y-6 md:p-6 lg:px-8 animate-fade-in">
      <Card padding="md">
        <CardHeader>
          ...
          {collections && <Pill tone="violet" size="sm">{collections.length} collections</Pill>}
        </CardHeader>
              ...
              <button
                ...
                className={`tap-target rounded-md border px-2 py-2 text-left text-caption transition-colors duration-fast ${
                  selected === c.name
                    ...
                    : "border-border-subtle bg-bg-elevated text-fg-primary hover:border-border-strong"
                }`}
              >
                <div className="font-bold truncate">{c.name}</div>
                <div className="text-micro text-fg-muted">{c.count.toLocaleString()} vectors</div>
              </button>
            ))}
          </div>
        )}
      ...
      {selected && (
        <Card padding="md">
          <CardHeader>
            ...
            <Pill tone={mode === "search" ? "cyan" : "violet"} size="sm">{mode}</Pill>
          </CardHeader>
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              className="flex-1 rounded-md border border-border-subtle bg-bg-elevated px-3 py-2 text-caption text-fg-primary"
            />
                ...
                >
                  <div className="flex items-center gap-2 text-micro text-fg-muted">
                    ...
                    {typeof it.distance === "number" && (
                      <Pill tone="violet" size="sm">d={it.distance.toFixed(3)}</Pill>
                    )}
                  </div>
                  {it.document && (
