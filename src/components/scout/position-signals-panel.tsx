...
type SortKey = "ticker" | "final_score" | "rs_pct" | "eps_yoy" | "rev_yoy" | "mcap";
...
const COLUMNS: ReadonlyArray<{ key: SortKey; label: string; align?: "left" | "right" }> = [
  ...
  { key: "eps_yoy", label: "EPS YoY", align: "right" },
  { key: "rev_yoy", label: "Rev YoY", align: "right" },
...
export function PositionSignalsPanel() {
  const signals = useScoutFetch(
    (signal) => fetchSignals("position", { limit: 50, signal }),
    [],
    { refreshMs: 60_000 },
  );
  const history = useScoutFetch(
    (signal) => fetchHistory("position", { days: 30, signal }),
    [],
    { refreshMs: 5 * 60_000 },
  );
    ...
    <Card padding="none" className="overflow-hidden">
      <header className="flex flex-wrap items-center justify-between gap-2 border-b border-border-subtle px-4 py-3">
        <div className="flex items-baseline gap-3">
          <h2 className="text-h3 text-fg-primary">ENGINE A</h2>
      ...
      <div className="overflow-x-auto">
        {signals.loading && !signals.data ? (
          ...
          <EmptyState
            ...
            body="Engine A has not run, or no tickers passed the Trend Template gate."
          />
        ) : (
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-2 py-1.5 text-right tabular-nums">{formatPct(c?.eps_yoy)}</td>
        <td className="px-2 py-1.5 text-right tabular-nums">{formatPct(c?.rev_yoy)}</td>
...
function PositionDetail({ signal }: { signal: Signal }) {
  const c = signal.components;
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2">
      <div>
        <div className="mb-1 text-micro uppercase tracking-wider text-fg-muted">
          Factor breakdown
        </div>
        <div className="flex flex-wrap gap-1.5">
          {scored.length ? (
            scored.map(([k, v]) => (
              <Pill key={k} tone="cyan" size="sm">
                {(FACTOR_LABEL[k] ?? k.toUpperCase())} +{v}
              </Pill>
            ))
          ) : (
            <span className="text-caption text-fg-muted">no factors scored</span>
          )}
        </div>
      ...
      <div>
        <div className="mb-1 text-micro uppercase tracking-wider text-fg-muted">Catalysts</div>
        <ul className="space-y-1 text-caption text-fg-primary">
          {d.insider && <li>· {d.insider}</li>}
          {d.filing && <li>· {d.filing}</li>}
          {d.stake && <li>· {d.stake}</li>}
          {!d.insider && !d.filing && !d.stake && (
            <li className="text-fg-muted">no specific catalyst details</li>
          )}
        </ul>
        {c.pattern && (
          <div className="mt-2 text-micro text-fg-muted">
            Pattern: <span className="text-fg-primary">{c.pattern}</span>
          </div>
        )}
        {signal.name && (
...
function pickSortValue(s: Signal, key: SortKey): number | string | null {
  ...
  if (key === "eps_yoy") return s.components?.eps_yoy ?? null;
  if (key === "rev_yoy") return s.components?.rev_yoy ?? null;
