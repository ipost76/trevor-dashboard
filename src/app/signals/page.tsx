...
import { Activity, RefreshCw, TrendingUp, TrendingDown, Target } from "lucide-react";
...
const BUCKET_LABELS: Record<string, string> = {
  under_35: "<35%", "35_44": "35-44%", "45_54": "45-54%", "55_64": "55-64%", "65_plus": "65%+",
};
        ...
        <div className="panel">
          ...
          <div className="p-2">
            ...
            {byTicker.slice(0, 12).map((t) => {
              const perf = tickerPerf.find(p => p.symbol === t.ticker);
              return (
                <div key={t.ticker} className="flex items-center gap-2 px-2 py-1.5 text-[10px] border-b border-[rgba(0,240,255,0.04)] hover:bg-[rgba(0,240,255,0.03)]">
                  ...
                  {tickerPerf.length > 0 && (
                    <span className={cn("w-14 text-right font-bold font-mono", perf ? wrColor(perf.winRate) : "text-muted-foreground")}>
                      {perf ? `${perf.winRate}%` : "—"}
                    </span>
                  )}
                </div>
