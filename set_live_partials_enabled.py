#!/usr/bin/env python3
"""
set_live_partials_enabled.py — Flag-gated toggle for LIVE_PARTIALS_ENABLED
backing POST /api/auto/partials-toggle (B4, 2026-05-27). Modeled exactly on
set_autotrader_enabled.py + set_confirm_cycles_promoted.py.

Usage:
    set_live_partials_enabled.py <true|false> <author>

Behavior:
    1. Refuse if HUB_LIVE_PARTIALS_TOGGLE_ENABLED != 'true' (exit 3).
    2. Read current auto_config.LIVE_PARTIALS_ENABLED. If matches requested →
       no-op (still exit 0, no audit row, no DB write).
    3. Otherwise, in a single transaction:
         a) UPSERT auto_config.LIVE_PARTIALS_ENABLED to 'true' or 'false'.
            auto_trader/config.cfg_bool() reads with no cache, so the new
            value is picked up at the next monitor cycle (~5s) with no
            service restart needed.
         b) INSERT autotrader_state_audit row reusing the existing table
            (same pattern B3 set for confirm_cycles_*) — the toggle surfaces
            in AutoTrader Control's "Recent Toggles" list for free.
            action = 'live_partials_on' or 'live_partials_off';
            source = f"hub:{author}".
...
Exit codes:
    ...
    3  toggle disabled (HUB_LIVE_PARTIALS_TOGGLE_ENABLED is false)

NOTE: Rule 1 + Rule 31 still binding — flipping ON only enables Layer 5
partial exits to actually execute (`execute_partial_live()` reduce-only
IOC). It does NOT close any open position, cancel any HL order, or
...
def _ensure_audit_table(conn: sqlite3.Connection) -> None:
    ...
    set_autotrader_enabled.py creates."""
    conn.execute("""
...
def main() -> None:
    if len(sys.argv) < 3:
        _emit({"ok": False, "error": "Usage: set_live_partials_enabled.py <true|false> <author>"}, code=1)
    ...
    try:
        with _conn_rw() as conn:
            row = conn.execute(
                "SELECT value FROM auto_config WHERE key='HUB_LIVE_PARTIALS_TOGGLE_ENABLED'"
            ).fetchone()
            ...
            conn.execute(
                ...
                ("LIVE_PARTIALS_ENABLED", requested_str),
            )

            action = "live_partials_on" if requested else "live_partials_off"
